...
class TestGetAllBuses:
    async def test_returns_buses(self, client):
        with aioresponses() as m:
            m.post(FLEET_URL, body=FLEET_ALL_XML)
    ...
    async def test_raises_on_network_error(self, client):
        with aioresponses() as m:
            m.post(FLEET_URL, exception=Exception("timeout"))
...
class TestGetRouteBuses:
    async def test_returns_buses(self, client):
        with aioresponses() as m:
            m.post(FLEET_URL, body=ROUTE_FLEET_XML)
...
class TestGetStopArrivals:
    async def test_returns_arrivals(self, client):
        with aioresponses() as m:
            m.get(ARRIVALS_URL, body=ARRIVALS_HTML)
            arrivals = await client.get_stop_arrivals("220602")
        assert len(arrivals) == 2

    async def test_returns_empty_on_500(self, client):
        with aioresponses() as m:
            m.get(ARRIVALS_URL, status=500)
            arrivals = await client.get_stop_arrivals("000000")
        assert arrivals == []


class TestGetRoutesAtStop:
    async def test_returns_route_set(self, client):
        with aioresponses() as m:
            m.get(ROUTES_AT_STOP_URL, body=ROUTES_BY_STATION_HTML)
            routes = await client.get_routes_at_stop("220602")
        assert "14M" in routes


class TestGetStopArrivalsVia:
    async def test_filters_to_common_routes(self, client):
        ...
        with aioresponses() as m:
            m.get(ROUTES_AT_STOP_URL, body=ROUTES_BY_STATION_HTML)   # origin: 14M, 15TY
            m.get(ROUTES_AT_STOP_URL, body=via_html)                  # via: 14M only
            m.get(ARRIVALS_URL, body=ARRIVALS_HTML)                   # 500T + 14M
            arrivals = await client.get_stop_arrivals_via("220602", "216572")
...
class TestGetRouteSchedule:
    async def test_returns_schedule(self, client):
        with aioresponses() as m:
            m.post(SCHEDULE_URL, body=SCHEDULE_XML)
            deps = await client.get_route_schedule("500T")
        assert deps[0].departure_time == "05:55"


class TestGetAnnouncements:
    async def test_returns_all(self, client):
        with aioresponses() as m:
            m.post(ANNOUNCEMENTS_URL, body=ANNOUNCEMENTS_XML)
            anns = await client.get_announcements()
        assert len(anns) == 1

    async def test_filter_by_hat_kodu(self, client):
        with aioresponses() as m:
            m.post(ANNOUNCEMENTS_URL, body=ANNOUNCEMENTS_XML)
...
class TestGetRouteStops:
    async def test_returns_stops(self, client):
        with aioresponses() as m:
            m.post(STOPS_URL, body=ROUTE_STOPS_XML)
            stops = await client.get_route_stops("500T")
        assert stops[0].stop_code == "301341"


class TestSearchStops:
    async def test_returns_stops_only(self, client):
        with aioresponses() as m:
            m.get(SEARCH_URL, payload=SEARCH_JSON)
