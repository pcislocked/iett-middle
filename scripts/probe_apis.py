...
async def probe_all():
    ...
    async with aiohttp.ClientSession(connector=connector) as s:
        ...
        status, xml = await soap_post(
            s,
            f"{IETT_SOAP}/FiloDurum/SeferGercaklesme.asmx",
        ...
        print(
            f"  HTTP {status} | records={len(data) if data else 'err'} | fields={list(data[0].keys()) if data else 'n/a'}"
        )
        ...
        print(
            f"  HTTP {status} | records={len(data) if data else 'err'} | sample={data[0] if data else 'n/a'}"
        )
        ...
        status, ct, html = await http_get(
            s, f"{IETT_REST}/GetStationInfo", {"dcode": "220602", "langid": "1"}
        )
        ...
        print(
            f"  HTTP {status} | arrivals={len(arrivals_raw)} | {[a['route'] for a in arrivals_raw[:5]]}"
        )

        print("── 3b. GetStationInfo dcode=220601 ──")
        status2, _, html2 = await http_get(
            s, f"{IETT_REST}/GetStationInfo", {"dcode": "220601", "langid": "1"}
        )
        ...
        status3, _, html3 = await http_get(
            s, f"{IETT_REST}/GetStationInfo", {"dcode": "216572", "langid": "1"}
        )
        ...
        status, xml = await soap_post(
            s,
            f"{IETT_SOAP}/UlasimAnaVeri/PlanlananSeferSaati.asmx",
        ...
        status, xml = await soap_post(
            s,
            f"{IETT_SOAP}/UlasimDinamikVeri/Duyurular.asmx",
        ...
        print(
            f"  HTTP {status} | stops={len(stops_raw)} | fields={list(stops_raw[0].keys()) if stops_raw else 'n/a'}"
        )

        print("── 7. GetSearchItems q=ahmet mithat ──")
        status, ct, body = await http_get(
            s, f"{IETT_REST}/GetSearchItems", {"key": "ahmet mithat", "langid": "1"}
        )
        try:
        ...
        status, ct, body = await http_get(
            s, f"{IETT_REST}/GetSearchItems", {"key": "500T", "langid": "1"}
        )
        try:
        ...
        status, ct, html = await http_get(
            s, f"{IETT_REST}/GetRouteByStation", {"dcode": "220602", "langid": "1"}
        )
        soup = BeautifulSoup(html, "html.parser")
        routes_at_stop = {
            sp.text.strip()
            for item in soup.select("div.line-item")
            for sp in item.select("a > span:first-child")
        }
        ...
        status, ct, body = await http_get(
            s, f"{IETT_REST}/GetStationForRoute", {"hatkod": "14M", "langid": "1"}
        )
        try:
            sfr = json.loads(body)
        except Exception:
            sfr = body[:200]
        summarise(
            "9_station_for_route_14M",
            status,
            sfr if isinstance(sfr, list) else [{"raw": str(sfr)[:200]}],
        )
        print(
            f"  HTTP {status} | ct={ct[:40]} | type={type(sfr).__name__} | len={len(sfr) if isinstance(sfr, list) else '?'}"
        )
        ...
        status, ct, body = await http_get(
            s, f"{IETT_REST}/GetFastStation", {"routeid": "14M", "langid": "1"}
        )
        try:
            fst = json.loads(body)
        except Exception:
        ...
        summarise(
            "10_fast_station_14M",
            status,
            fst if isinstance(fst, list) else [{"raw": str(fst)[:200]}],
        )
        print(
            f"  HTTP {status} | type={type(fst).__name__} | len={len(fst) if isinstance(fst, list) else '?'}"
        )
        ...
        summarise(
            "11_all_route_14M",
            status,
            ar if isinstance(ar, list) else [{"raw": str(ar)[:300]}],
        )
        print(f"  HTTP {status} | type={type(ar).__name__} | data={ar}")

        print("── 12. GetRouteStation key empty ──")
        status, ct, body = await http_get(
            s, f"{IETT_REST}/GetRouteStation", {"key": "", "langid": "1"}
        )
        try:
            rs = json.loads(body)
        ...
        print(
            f"  HTTP {status} | ct={ct[:40]} | size={size_kb:.1f}kB | records={len(segs) if isinstance(segs, list) else '?'}"
        )
        ...
        summarise(
            "15_tunnel_segments",
            status,
            tun if isinstance(tun, list) else [{"raw": body[:200]}],
        )
        ...
        routes_220602 = {
            ...
            for item in BeautifulSoup(
                (
                    await http_get(
                        s,
                        f"{IETT_REST}/GetRouteByStation",
                        {"dcode": "220602", "langid": "1"},
                    )
                )[2],
                "html.parser",
            ).select("div.line-item")
            for sp in item.select("a > span:first-child")
        }
        routes_216572 = {
            sp.text.strip()
        ...
        print(
            f"  origin routes={len(routes_220602)} via routes={len(routes_216572)} common={sorted(common)}"
        )
